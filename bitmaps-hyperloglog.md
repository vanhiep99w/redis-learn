...
Phản xạ đầu tiên là dùng [Set](./sets.md): mỗi user là một member, `SCARD` để đếm, `SISMEMBER` để hỏi từng người.
...
- **HyperLogLog** — chỉ ước lượng số lượng (sai số ~0.81%), không hỏi được cá nhân, nhưng mỗi key tối đa chỉ ~12KB dù đếm tới hàng tỉ phần tử.
...
|---|---|---|
| Bản chất | [String](./strings.md) dùng như mảng bit | Probabilistic cardinality structure lưu trong String |
| Kết quả | Chính xác 100% | Ước lượng, standard error ~**0.81%** |
| Memory | `offset_lớn_nhất / 8` bytes, tối đa 512MB | Sparse vài byte/KB, dense tối đa ~12KB |
...
└─ HLL: bytes là registers thống kê sau khi hash → mất danh tính phần tử, chỉ còn ước lượng cardinality
```
...
Bitmap **không phải data type riêng**. Redis dùng một String binary-safe, rồi cho phép đọc/ghi từng bit bằng offset.
...
> [!WARNING]
> `SETBIT key 4000000000 1` trên key rỗng không lưu “một bit thưa”. Redis phải mở rộng String đến offset đó và pad toàn bộ khoảng trống bằng `0`, tức khoảng **500MB**. Lệnh này có thể block event loop trong lúc cấp phát. Nếu ID là UUID/snowflake/thưa, đừng dùng trực tiếp làm offset.
...
HyperLogLog trả lời một câu duy nhất: **tập này có khoảng bao nhiêu phần tử distinct?** Nó không lưu phần tử. Nó lưu “dấu vết thống kê” sau khi hash.
...
Một hash có 18 zero liên tiếp ở phần rank là sự kiện hiếm; nếu đã thấy nó, cardinality chắc không nhỏ. Nhưng một register quá nhiễu, nên HLL dùng **16,384 registers** rồi lấy trung bình điều hòa (harmonic mean) kèm bias correction.
