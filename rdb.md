...
RDB (Redis Database) là cơ chế **point-in-time snapshot** (ảnh chụp tại đúng một thời điểm): Redis ghi lại **toàn bộ** key-value hiện có trong bộ nhớ ra một file binary duy nhất, mặc định là `dump.rdb`.
...
| **Replica bootstrap** | Master gửi RDB cho replica khi full sync (đồng bộ toàn bộ dataset từ đầu) | Xem [Replication](./replication.md) |
...
Redis có hai lệnh tạo snapshot, khác nhau ở **một điểm sống còn**: có chặn event loop (vòng xử lý lệnh chính của Redis) hay không. Nói đơn giản: một cách bắt Redis đứng yên để chụp, cách còn lại nhờ process con chụp thay.
...
`BGSAVE` là kiểu "gọi người phụ chụp ảnh": Redis chính vẫn bán hàng, còn process con lo ghi ảnh ra đĩa.

`BGSAVE` (Background Save) dùng `fork()` (system call tạo tiến trình con trên Unix/Linux) để tạo một **tiến trình con**, con lo việc ghi file, còn tiến trình cha (main) tiếp tục phục vụ request bình thường nhờ Copy-on-Write (CoW — chỉ copy trang nhớ khi có ghi).

```diagram
Main process ──fork()──┬──▶ Child process ──[ ghi dump.rdb ]──▶ exit
       │               │
       │               └─ chia sẻ memory qua Copy-on-Write (CoW)
       ▼
...
| Rủi ro | Treo server | Tốn RAM lúc write nhiều (Copy-on-Write / CoW) |
...
Đây là phần "aha" quan trọng nhất của RDB. Làm sao tiến trình con ghi được một snapshot **nhất quán** của 10GB dữ liệu, trong khi tiến trình cha vẫn đang liên tục sửa dữ liệu đó? Câu trả lời là: `fork()` tạo một góc nhìn đóng băng, còn CoW giữ cho góc nhìn đó không bị thay đổi.

### 5.1 fork() không copy 10GB RAM

`fork()` quan trọng vì nó quyết định `BGSAVE` có khởi động nhanh hay biến thành một lần copy RAM khổng lồ.
...
Copy-on-Write là mảnh ghép giúp snapshot vừa nhất quán vừa không phải dừng write của Redis.
...
Đây là phần cần nhớ khi sizing RAM: `BGSAVE` có thể chạy nền, nhưng write-heavy workload vẫn phải trả tiền bằng bộ nhớ.
...
> [!TIP]
> Bật Transparent Huge Pages (THP — cơ chế kernel gộp page memory lớn để tối ưu một số workload) làm CoW copy theo trang 2MB thay vì 4KB → khuếch đại RAM và latency. Redis khuyến cáo **tắt THP** (`madvise`/`never`). Log khởi động Redis sẽ cảnh báo nếu THP đang bật.
...
### Khi nào KHÔNG nên dùng (chỉ) RDB

- Dữ liệu không được mất như tài chính, ví tiền, order quan trọng → cần [AOF](./aof.md) với `appendfsync everysec` hoặc `always`.
- Nếu mất cửa sổ giữa 2 snapshot là không chấp nhận được → RDB đơn thuần không đủ, nên dùng AOF hoặc hybrid theo [Persistence Strategies](./persistence-strategies.md).
- Cần độ bền khoảng ~1 giây sau mỗi write → chọn AOF `everysec`; cần chặt hơn nữa thì cân nhắc `always` và chấp nhận chi phí latency.
- Nếu chỉ là cache warm-lại-được, mất vài phút không sao → RDB (hoặc thậm chí no persistence) thường là đủ.
