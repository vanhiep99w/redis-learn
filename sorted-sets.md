...
- Đếm unique rất lớn, chỉ cần ước lượng → dùng [HyperLogLog](./bitmaps-hyperloglog.md).
...
| `ZUNIONSTORE` nhiều ZSet lớn trong request path | CPU spike, **block event loop** ([Redis Architecture](./redis-architecture.md)) | Materialize định kỳ/background, cache kết quả |
